...
The application SHALL call `AddIdentity<IdentityUser, IdentityRole>()` (or `AddDefaultIdentity`) chained with `.AddEntityFrameworkStores<OrderDbContext>()` during service registration, so that `UserManager<IdentityUser>`, `RoleManager<IdentityRole>`, and `SignInManager<IdentityUser>` are resolvable from the DI container.
